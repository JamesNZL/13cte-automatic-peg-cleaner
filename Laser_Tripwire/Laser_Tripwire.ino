...
const sensorPin = A0;                                       // LDR 'signal' pin
const testIterations = 5;                                   // number of readings to perform when initialising
const armedValue = 1000;                                    // maximum reading possible from LDR, therefore reading when laser is 'armed'
...
void loop() {
  ...
  while (sensorValue >= triggerValue) {                     // until the analogue reading has dropped below 'activation' threshold, do the following
  ...
  Serial.println("Triggered");                              // if tripwire is broken, print 'Triggered' to serial monitor
}
